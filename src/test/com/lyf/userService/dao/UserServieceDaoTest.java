...
public class UserServieceDaoTest extends BaseTest {
    ...
    @Autowired
    private UserServiceDao userServieceDao;

    @Test
    public void insert() throws Exception {

        List<UserService> userServieceArrayList = new ArrayList<>();
        ...
        for (CompanyCardAudit cardAudit : companyCardAuditList) {
            UserService userServiece = new UserService();
            userServiece.setId("id" + cardAudit.getCode() + cardAudit.getTaxid());
            userServiece.setTaxId(cardAudit.getTaxid());
            userServiece.setServiceId(serviceIds[(i++) * i % 150]);
//            shopBuyer.setAddDate(new Date());
            userServieceArrayList.add(userServiece);
        }
        DataSourceContextHolder.setDBType("dataSourceUserService");
        userServieceDao.insert(userServieceArrayList);
    }
}